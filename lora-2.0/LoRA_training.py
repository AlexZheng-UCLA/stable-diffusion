...
# model
training_ckpt = "/root/autodl-tmp/webui_models/Stable-diffusion/chilloutmix_NiPrunedFp32Fix.safetensors"

vae = "/root/autodl-tmp/webui_models/VAE/vae-ft-mse-840000-ema-pruned.safetensors"  
blip = "/root/autodl-tmp/webui_models/BLIP/model_large_caption.pth"
...
root_dir = "/root/stable-diffusion/lora-2.0"
v2 = False 
...
training_dir = os.path.join(root_dir, "workspace", dir_name)
...
if data_anotation == "blip" or data_anotation == "combined":
    ...
    command = f'''python make_captions.py "{train_data_dir}" --caption_weights {blip} --batch_size {batch_size} {"--beam_search" if beam_search else ""} --min_length {min_length} --max_length {max_length} --caption_extension .caption --max_data_loader_n_workers {max_data_loader_n_workers}'''
